<?php 

    $pg = "proyectos";

?>

<!DOCTYPE html>
<html lang="es" class="h-100">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/fontawesome/css/all.min.css">
    <link rel="stylesheet" href="css/fontawesome/css/fontawesome.min.css">
    <link rel="shortcut icon" href="images/ep.ico" type="image/x-icon">
    <link rel="stylesheet" href="css/bootstrap/css/bootstrap.min.css">
    <script src="css/bootstrap/js/bootstrap.bundle.min.js"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4bw+/aepP/YC94hEpVNVgiZdgIC5+VKNBQNGCHeKRQN+PtmoHDEXuppvnDJzQIu9" crossorigin="anonymous">
    <title>Proyectos</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body id="proyectos" class="d-flex flex-column h-100">
    <header class="container">
        <?php include_once("menu.php") ?>
    </header>
    <main class="container">
        <div class="row">
            <div class="col-12 pt-3 pb-5">
                <h1>Proyectos</h1>
            </div>
        </div>
        <div class="row">
            <div class="col-12 mb-3">
                <p>Los siguientes son algunos de los trabajos que he realizado:</p>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-4 col-12 mb-3">
                <div class="proyecto shadow">
                    <div class="row">
                        <div class="col-12">
                            <img src="images/abmclientes.png" alt="ABM Clientes" class="img-fluid">
                        </div>
                        <div class="col-12">
                            <h2 class="px-3 py-2">ABM CLIENTES</h2>
                        </div>
                        <div class="col-12">
                            <p class="px-3 py-1">Alta, baja y modificación de un registro de clientes. Realizado en HTML, CSS, PHP, Bootstrap y Json.</p>
                        </div>
                        <div class="col-12">
                            <div class="row mt-3 py-4 px-3">
                                <div class="col-6">
                                    <a href="#" class="btn btnrojo">Ver online</a>
                                </div>
                                <div class="col-6 text-center">
                                    <a href="#" class="linkrojo">Código fuente</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 col-12 mb-3">
                <div class="proyecto shadow">
                    <div class="row">
                        <div class="col-12">
                            <img src="images/abmventas.png" alt="AMB Ventas" class="img-fluid">
                        </div>
                        <div class="col-12">
                            <h2 class="px-3 py-2">SISTEMA DE GESTIÓN DE VENTAS</h2>
                        </div>
                        <div class="col-12">
                            <p class="px-3 py-1">Sistema de gestión de clientes, productos y ventas. Realizado en HTML, CSS, PHP, MVC, Bootstrap, Js, Ajax, jQuery y MySQL de base de datos.</p>
                        </div>
                        <div class="col-12">
                            <div class="row mt-3 py-4 px-3">
                                <div class="col-6">
                                    <a href="#" class="btn btnrojo">Ver online</a>
                                </div>
                                <div class="col-6 text-center">
                                    <a href="#" class="linkrojo">Código fuente</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 col-12 mb-3">
                <div class="proyecto shadow">
                    <div class="row">
                        <div class="col-12">
                            <img src="images/proyecto-integrador.png" alt="Proyecto integrador" class="img-fluid">
                        </div>
                        <div class="col-12">
                            <h2 class="px-3 py-2">PROYECTO INTEGRADOR</h2>
                        </div>
                        <div class="col-12">
                            <p class="px-3 pb-3 py-1">Proyecto Full Stack desarrollado en PHP, Laravel, Javascript, jQuery, AJAX, HTML, CSS, Mercadopago con panel administrador, gestor de usuarios, módulo de permisos y funcionalidades a fines.</p>
                        </div>
                        <div class="col-12">
                            <div class="row mt-3 py-4 px-3">
                                <div class="col-6">
                                   <a href="#" class="btn btnrojo">Ver online</a>
                                </div>
                                <div class="col-6 text-center">
                                    <a href="#" class="linkrojo">Código fuente</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
    <footer class="container mt-auto mb-4 footerojo">
        <div class="btnwts">
            <a href="https://api.whatsapp.com/send/?phone=543442646995&text&type=phone_number&app_absent=0" target="_blank"><i class="fa-brands fa-whatsapp" style="color: #ffffff"></i></a>
        </div>
        <div class="row">
            <div class="col-sm-3 col-12">
                <a href="https://github.com/EmiPicart" target="_blank"><i class="fa-brands fa-github"></i></a>
                <a href="https://www.linkedin.com/in/emiliano-picart-049b3427a/" target="_blank"><i class="fa-brands fa-linkedin-in"></i></a>
            </div>
            <div class="col-sm-3 col-12">
                Sponsor <a href="https://depcsuite.com">DePC Suite</a>
            </div>
            <div class="col-sm-3 col-12">
                <a href="mailto:emilianopicart1@hotmail.com">emilianopicart1@hotmail.com</a>
            </div>
        </div>
    </footer>
</body>
</html>